...
ALPAKA_FN_ACC ALPAKA_FN_INLINE void atomicMaxPair(const TAcc& acc,
  ...
  while (comparator(value,std::pair<unsigned int, float>{static_cast<unsigned int>(ret >> 32 & 0xffffffff),
...
ALPAKA_FN_ACC ALPAKA_FN_INLINE void atomicMaxPair(const TAcc& acc,
  ...
  unsigned long long int val = (static_cast<unsigned long long int>(value.first) << 32) + edm::bit_cast<unsigned int>(value.second);
  unsigned long long int ret = *address;
  while (comparator(value,std::pair{static_cast<unsigned int>(ret >> 32 & 0xffffffff),
